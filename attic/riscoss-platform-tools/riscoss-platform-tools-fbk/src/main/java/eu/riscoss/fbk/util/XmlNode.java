...
public class XmlNode implements CharSequence, Comparable<XmlNode>, Iterable<XmlNode>
{
	 ...
	 * @param file
	 *            - file to be loaded
	 * @return an XmlNode hierarcy or NoNode if an error occurs during
	 */
	...
	public static XmlNode loadString( String string )
	{
		...
		try
		{
			documentBuilderFactory = DocumentBuilderFactory.newInstance();
			documentBuilder = documentBuilderFactory.newDocumentBuilder();
			document = documentBuilder.parse( new ByteArrayInputStream( string.getBytes( "UTF-8" ) ) );
		}
		catch( Exception e )
		{
			return s_noneNode;
	...
	ArrayMap<String, String>		m_attr		= new ArrayMap<String, String>();

	MultiMap<String, XmlNode>	children	= new MultiMap<String, XmlNode>();

	/*
	 * public static XmlNode create( String filename ) { Document document =
	 * null; DocumentBuilder documentBuilder = null; DocumentBuilderFactory
	 ...
	 * documentBuilder.parse( new File( filename ) ); } catch(Exception e) {
	 * e.printStackTrace();
	 * 
	 * return null; }
	 * 
